...
router.delete('/cliente/:micli', async(req, res) => {
    ...
    const respuesta = await db.query('delete from clientes where id_cliente = ?', [id], (err, rows) => {

      if (!err){
         res.json('Se elimino exitosamente el cliente');
      }else{
         res.json('No se pudo eliminar el cliente');
      }
    });
...
router.put('/cliente/:id_cli', async(req, res) => {
 
    let id = req.params.id_cli;
    let unCli = req.body;
    
    const respuesta = await db.query('update clientes set ? where id_cliente = ?',
  [unCli, id]);
  res.json(respuesta);
});
